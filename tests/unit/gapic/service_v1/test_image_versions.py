...
def test_image_versions_client_client_options_from_dict():
    with mock.patch(
        "google.cloud.orchestration.airflow.service_v1.services.image_versions.transports.ImageVersionsGrpcTransport.__init__"
    ) as grpc_transport:
...
async def test_list_image_versions_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_image_versions(
            request={},
        )
...
def test_image_versions_base_transport():
    ...
    with mock.patch(
        "google.cloud.orchestration.airflow.service_v1.services.image_versions.transports.ImageVersionsTransport.__init__"
    ) as Transport:
...
def test_image_versions_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.cloud.orchestration.airflow.service_v1.services.image_versions.transports.ImageVersionsTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
